...
const App = () => {
  ...
  const [notification, setNotification] = useState({message: null, isError:});
  ...
  const deletePerson = (id) => {
    ...
    if (confirm(`Delete ${matchPerson.name}?`))
      PersonServices.remove(id).then((deletedPerson) => {
        setPersons(persons.filter((person) => person.id !== deletedPerson.id));
        handleNotification(`Deleted ${deletedPerson.name}`)
      }).catch((error) => {
        handleNotification(`${matchPerson.name} has already been removed from the system`, isError=true)
